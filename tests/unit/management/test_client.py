...
def _make_metrics_client():
    from smplkit.management.client import MetricsClient

    parent = MagicMock()
    app_http = MagicMock()
    return MetricsClient(parent, app_http)


def _make_async_metrics_client():
    from smplkit.management.client import AsyncMetricsClient

    parent = MagicMock()
    app_http = MagicMock()
    return AsyncMetricsClient(parent, app_http)


class TestMetricsClient:
    def test_bulk_ingest_empty_list_is_noop(self):
        from smplkit.management.client import MetricsClient

        with patch("smplkit.management.client._gen_bulk_ingest_metrics.sync_detailed") as mock_post:
            client = _make_metrics_client()
            client.bulk_ingest([])
            mock_post.assert_not_called()
            _ = MetricsClient  # keep the import warm for ruff

    @patch("smplkit.management.client._gen_bulk_ingest_metrics.sync_detailed")
    def test_bulk_ingest_serializes_items(self, mock_post):
        ...
        assert attrs.name == "flags.evaluations"
        assert attrs.value == 1482
        assert attrs.unit == "evaluations"

    @patch("smplkit.management.client._gen_bulk_ingest_metrics.sync_detailed")
    def test_bulk_ingest_raises_on_error(self, mock_post):
        import datetime as _dt
...
class TestAsyncMetricsClient:
    def test_bulk_ingest_empty_list_is_noop_async(self):
        import asyncio
