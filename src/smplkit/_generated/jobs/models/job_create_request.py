...
class JobCreateRequest:
    ...
    Attributes:
        data (JobCreateResource): JSON:API resource envelope for creating a job (id required). Example: {'attributes':
            {'concurrency_policy': 'ALLOW', 'configuration': {'body': '{"scope":"all"}', 'headers': {'Authorization':
            'Bearer s3cr3t'}, 'method': 'POST', 'success_status': '2xx', 'timeout': 30, 'tls_verify': True, 'url':
            'https://api.example.com/cache/warm'}, 'description': 'Warms the product cache every night at 02:00 New York
            time.', 'environments': {'production': {'enabled': True}, 'staging': {'enabled': True, 'headers.Authorization':
            'Bearer staging', 'schedule': '0 3 * * *', 'timezone': 'Europe/London', 'url':
