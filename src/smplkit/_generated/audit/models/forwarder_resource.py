    ...
    The caller supplies `id` (the forwarder's key) on create.
        ...
        Example:
            {'attributes': {'configuration': {'headers': {'Content-Type': 'application/json', 'DD-API-KEY': 'dd-api-key-
                ...
                events to the prod Datadog tenant.', 'environments': {'production': {'enabled': True, 'headers.DD-API-KEY': 'dd-
                prod-api-key-plaintext'}}, 'filter': {'==': [{'var': 'event_type'}, 'user.created']}, 'forward_smplkit_events':
                False, 'forwarder_type': 'datadog', 'name': 'Datadog production', 'transform': '{ "message": event_type & \' on
