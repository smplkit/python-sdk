...
class ForwarderCreateResource:
    ...
    Example:
        {'attributes': {'configuration': {'headers': [{'name': 'Content-Type', 'value': 'application/json'}, {'name':
            ...
            Datadog tenant.', 'environments': {'production': {'enabled': True, 'headers.DD-API-KEY': 'dd-prod-api-key-
            plaintext'}}, 'filter': {'==': [{'var': 'event_type'}, 'user.created']}, 'forward_smplkit_events': False,
            'forwarder_type': 'datadog', 'name': 'Datadog production', 'transform': '{ "message": event_type & \' on \' &
            resource_type }', 'transform_type': 'JSONATA'}, 'id': 'datadog-prod', 'type': 'forwarder'}
